...
export default function AdminItemStats({ blockProps, title }: Props) {
  const { useFetchDetail, apiClient, jsxBackend } = useGlobal();
  ...
  return (
    <Block>
      <BlockContentWrapper>
        <HeaderWrapper>
          <HeaderLeftWrapper>
            <TitleHeader> Welcome back, {session.user.full_name} </TitleHeader>
            <Time>
              {!loading ? (
                <>
        ...
        <Chart className={loading ? 'loading' : ''}>
          <>
            ...
            <Line width={500} options={options} data={data} />
          </>
        </Chart>
      </BlockContentWrapper>
    </Block>
