...
function* editCurrentSeo() {
  const { getPageParams, dialogBackend } = yield* getGlobalContext();

  const { pageMetaName } = getPageParams();

  if (!pageMetaName) return;
  ...
  yield call(dialogBackend.present, {
    ...
    props: {
      dataSource: {
        ...
        apiParams: {
          name: uniqMetaName(pageMetaName)
        }
      }
    }
  });
